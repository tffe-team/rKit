...
module.exports = class Service {
  ...
  init (mode = process.env.VUE_CLI_MODE) {
    ...
    //set hooks 
    if(this.projectOptions.hooks) {
      Object.assign(this.hooksConfig, this.projectOptions.hooks)
    }
  }
  ...
  resolveWebpackConfig (chainableConfig = this.resolveChainableWebpackConfig()) {
    ...
    if (
      ...
      throw new Error(
        ...
        `Use the "publicPath" option in rkit.config.js instead.`
      )
    }
  ...
  loadUserOptions () {
    ...
    const configPath = (
      ...
      path.resolve(this.context, 'rkit.config.js')
    )
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (!fileConfig || typeof fileConfig !== 'object') {
          error(
            `Error loading ${chalk.bold('rkit.config.js')}: should export an object or a function that returns object.`
          )
          fileConfig = null
        }
      } catch (e) {
        error(`Error loading ${chalk.bold('rkit.config.js')}:`)
        throw e
      }
    }
    ...
    if (fileConfig) {
      if (pkgConfig) {
        warn(
          ...
          `due to presence of ${chalk.bold('rkit.config.js')}.`
        )
        warn(
          `You should migrate it into ${chalk.bold('rkit.config.js')} ` +
          `and remove it from package.json.`
        )
      }
      resolved = fileConfig
      resolvedFrom = 'rkit.config.js'
    } else if (pkgConfig) {
      resolved = pkgConfig
    ...
    if (typeof resolved.baseUrl !== 'undefined') {
      if (typeof resolved.publicPath !== 'undefined') {
        warn(
          `You have set both "baseUrl" and "publicPath" in ${chalk.bold('rkit.config.js')}, ` +
          `in this case, "baseUrl" will be ignored in favor of "publicPath".`
        )
      } else {
        warn(
          `"baseUrl" option in ${chalk.bold('rkit.config.js')} ` +
