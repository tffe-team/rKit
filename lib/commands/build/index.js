...
module.exports = (api, options) => {
    api.registerCommand('build', {
        ...
        const getBuildBranch = require('../../util/getBuildBranch')
        const  branchs = await getBuildBranch()
        await prompt([{
            type: 'list',
            message: 'Please select a build branch',
            name: 'line',
            choices: branchs,
          }]).then(function(answers) {
              options.assetsDir = md5(answers)
...
async function build(args, api, options) {
    ...
    const forceCodeReview = require('../../util/forceCodeReview').forceCodeReview
    ...
    return new Promise((resolve, reject) => {
        webpack(webpackConfig, (err, stats) => {
            ...
            if (!args.silent) {
                ...
                log(formatStats(stats, targetDirShort, api, options))
