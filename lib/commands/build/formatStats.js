module.exports = function formatStats(stats, dir, api, options) {
        ...
        function createJsonFile(fileinfo, content) {
            fsExt.writeJson(fileinfo, content)
                .then(() => {
            ...
            assets.map(asset => makeRow(
                (() => {
                    if (/js$/.test(asset.name)) {
                        ...
                        return chalk.green(path.join(dir, asset.name))
                    } else {
        ...
        createJsonFile(path.join(dir, 'staticPath.json'), staticPathInfo)
        createJsonFile(path.join(dir, options.assetsDir + '/config.json'), configInfo)
